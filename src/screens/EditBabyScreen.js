...
export default function EditBabyScreen({ navigation, route }) {
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      await updateBabyProfile({ name, gender, birthday, avatarEmoji, nextCheckup });
      navigation.goBack();
    } catch (err) {
...
const styles = StyleSheet.create({
  ...
  formCard: { backgroundColor: '#fff', borderRadius: 14, padding: 4 },
  formRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 14, paddingHorizontal: 12 },
  formLabel: { fontSize: 16, color: '#333', width: 80 },
